 ...
 */
async function callAlfredServiceGet(apiURL, body, retryCounter, noRetry) {
  const options = {
    method: 'GET',
    uri: apiURL,
    json: true,
  ...
  try {
    ...
    if (!noRetry) {
      ...
      retryCount += 1;
      setTimeout(() => {
        callAlfredServiceGet(apiURL, body, retryCount);
      }, 60000); // 1 minute delay before re-tyring
    }
    return err;
  }
}
exports.callAlfredServiceGet = async (apiURL, body, noRetry) => {
  const apiResponse = await callAlfredServiceGet(apiURL, body, 0, noRetry);
  return apiResponse;
};
