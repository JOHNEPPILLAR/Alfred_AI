 ...
 */
exports.requestAPIdata = (apiURL, userAgent) => {
  try {
    const options = {
...
exports.zeroFill = (number, width) => zeroFill(number, width);
...
exports.inHomeGeoFence = function FnInHomeGeoFence(lat, long) {
  ...
  return geolib.isPointInside({ latitude: lat, longitude: long }, geoFenceHomeData);
};

exports.inJPWorkGeoFence = function FnInJPWorkGeoFence(lat, long) {
  const geoHomeFile = './geoJPWork.json';
  const geoFenceHomeData = JSON.parse(fs.readFileSync(geoHomeFile, 'utf8'));
  return geolib.isPointInside({ latitude: lat, longitude: long }, geoFenceHomeData);
};
...
exports.timeDiff = (startTime, timeFromNow, addMinutes) => {
  const endTime = moment(timeFromNow, 'HH:mm');
  let tmpStartTime = moment();
  if (startTime !== null) tmpStartTime = moment(startTime, 'HH:mm');
  let addMinutesToTime = 0;
  if (typeof addMinutes !== 'undefined') addMinutesToTime = addMinutes;
  endTime.add(addMinutesToTime, 'minutes');

  const duration = moment.duration(endTime.diff(tmpStartTime));
