...
async function callAlfredServicePut(apiURL, body, retryCounter) {
  ...
  try {
    ...
    log('error', 'callAlfredServicePut', err.message);

    let errorCode = err.cause;
    if (typeof errorCode !== 'undefined' && errorCode !== null) errorCode = err.cause.code;
    let retryCount = retryCounter || 0;
    if (errorCode === 'ECONNREFUSED' || errorCode === 'ResourceNotFound') {
...
async function callAPIService(apiURL, body) {
  ...
  try {
    ...
    log('error', 'callAPIService', err.message);
    return err;
  }
}
