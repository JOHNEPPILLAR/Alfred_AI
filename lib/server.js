...
server.use((req, res, next) => {
  ...
  res.header('Expires', '-1');
  res.header('Pragma', 'no-cache');
  if ((req.headers.app_key !== process.env.app_key) && req.url !== '/ping') {
...
const notificationsRouter = require('../skills/notifications/notifications.js');
...
notificationsRouter.applyRoutes(server, '/notifications');
