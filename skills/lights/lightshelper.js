...
exports.lightstate = function lightstate(res, lightNumber) {
    return getLightState(res, lightNumber)

    async function getLightState(res, lightNumber) {
        try {
            var state = await Hue.lightStatus(lightNumber)

            if (typeof res !== 'undefined' && res !== null) {
                alfredHelper.sendResponse(res, 'sucess', state); // Send response back to caller
            };
            return state;
        } catch(err) {
            if (typeof res !== 'undefined' && res !== null) {
